...
- **Фаза 1 (тестирование):** owner запускает этот промт несколько раз в отдельных чатах с РАЗНЫМИ версиями `09_reception_algorithm.md` (например v19 → v20 → v21). Цель: сравнить результаты, выбрать лучшую версию.
- **Фаза 2 (фиксация):** после выбора canonical версии — owner оставляет её как `09_reception_algorithm.md` в репо.
...
ALGORITHM_FILE_PATH = "/Users/andriy/Documents/espafloria.odoo/kb/reception_algorithm.md"
...
4. `/Users/andriy/Documents/espafloria.odoo/kb/reception_action_1217.py` — server action 1217 (mirror prod-кода). Понять что делает finalize-флаг.
...
Ищи в `/Users/andriy/Documents/espafloria.odoo/pedido.files/reception_paper/verdnatura_{PEDIDO_DOCNUM}.pdf`.

```bash
ls /Users/andriy/Documents/espafloria.odoo/pedido.files/reception_paper/verdnatura_{PEDIDO_DOCNUM}.pdf
```
...
Парсинг paper:
```bash
pdftotext -layout /Users/andriy/Documents/espafloria.odoo/pedido.files/reception_paper/verdnatura_{PEDIDO_DOCNUM}.pdf -
```
...
Если в репо есть прошлый run-report для того же pedido через COMPARE_TO_VERSION (поищи `kb/algo_test_runs/{PEDIDO_DOCNUM}_v*.md`) — покажи **side-by-side diff**:
...
Создай файл:
```
/Users/andriy/Documents/espafloria.odoo/kb/algo_test_runs/{PEDIDO_DOCNUM}_{ALGORITHM_VERSION}.md
```
...
После test run — одна строка в `/Users/andriy/Documents/espafloria.odoo/kb/CHANGELOG.md` сверху (bump v):

```
- 2026-05-XX — **algo test run pedido {PEDIDO_DOCNUM} v{ALGORITHM_VERSION}**: {DRY_RUN/LIVE}, paper {YY.YY}€ ≡ Odoo, K карт create, M reassigns. Issues: {краткое}. Report: kb/algo_test_runs/{PEDIDO_DOCNUM}_v{N}.md.
```
