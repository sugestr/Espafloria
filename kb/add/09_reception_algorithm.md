<!-- v: 19 | updated: 2026-05-02T21:30Z -->
# Verdnatura Reception — Agent Specification

> 🟡 **STATUS 2026-05-02 (post-reset):** база Odoo обнулена (все pedidos/sales/stock стёрты, каталог переимпортирован из Holded). Текущая v19 morally **требует verify** на чистом каталоге — некоторые правила опираются на pre-reset state (§A5 «existing robo-cards», migrated v2.2 cards). Owner планирует следующей сессией: (a) сравнить v1 vs v19, (b) тестовый прогон через `09_reception_INSTR_test_run.md` на 1 pedido, (c) выпуск v20 если потребуется.
...
**Lineage:** этот документ — **единственный self-contained источник** для агента-приёмщика. Не требует чтения memory/ или других md-файлов. Базируется на Make.com bot prompt v3.5 (модуль 149), интегрирует reception_handover_2026-04-29.md, накопленные decisions из supervisor pilot 2026-04-30, и audit findings 2026-04-30.
...
- `paper_pdf_path` — `/Users/andriy/Documents/espafloria.odoo/kb/pedido.paper/verdnatura_<docNum>.pdf` (доступен через `mcp__workspace__bash` `pdftotext`)
- `paper_pdf_url` (для attach) — `https://raw.githubusercontent.com/sugestr/Espafloria/main/pedido.files/reception_paper/verdnatura_<docNum>.pdf`
...
Полная prod-логика action 1146 (`review_status_automation`) — в `kb/add/03_review_status_automation.py` (contract mirror, см. §H runtime checklist). Agent сам пишет color на `stock.move.x_studio_review_color`; action 1146 переключает текстовый `review_status` по триггерам.
...
- [ ] У меня есть доступ к: (a) этот файл (`09_reception_algorithm.md`), (b) `09_reception_action_1217.py` (mirror action 1217 в проде, v7.7 contract), (c) `03_review_status_automation.py` (mirror action 1146 в проде, color/review_status logic — referenced from §B7)
- [ ] paper PDF доступен локально на `reception_paper/verdnatura_<docNum>.pdf`
