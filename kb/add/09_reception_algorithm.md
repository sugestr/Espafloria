...
| `standard_price` | **PACK-AWARE (v22.1, 2026-05-04 owner directive):** для pack products (supplier UD VENTA = Paquete) — `paper.PVP / uds_per_paquete` (per Tallo basis, потому что template.uom_id = 1 Tallo). Для stem products (UD VENTA = Tallo) — `paper.PVP` напрямую. **Никогда** не писать `paper.PVP` per Tallo если supplier продаёт Paquete — это даёт ×N завышенный stock_quant valuation. Для recount-aware: использовать `paper.PVP / actual_uds_per_paquete_recount` если known (e.g., бот пересчитал 17 stems на 2 paquetes → 8.5 stems/paquete → cost per Tallo = paper.PVP / 8.5). Supplierinfo всегда хранит per Paquete uom (paper.PVP/u as-is). Не оставляем 0 — baseline нужен для list_price. |
