...
#### Decision rule по qty (карточка ОК)
| Случай | stock.move.quantity | review_color |
|---|---|---|
| Pack | florist stems (expected_qty) | оранжевый 🟠 |
| Stem direct (paper = florist) | paper (= florist) | зелёный ✅ |
| Stem small delta (≤5) | florist | жёлтый 🟡 |
| Stem big delta при правильной карточке | florist (default правило) | жёлтый 🟡 |
| Stem ×N >2 без pack signals | flag | красный ❌ |
| ×2 ровно | flag | красный ❌ |
...
| image_1920 | `set_binary_field` с `https://cdn.verdnatura.es/image/catalog/1600x900/<paper.ref>` (404 fallback OK) |
...
### 6.1 Pre-flight (before trigger)
- Все строки имеют не-красные предполагаемые бейджи (синий / зелёный / жёлтый / оранжевый).
- `sum(line.product_qty × line.price_unit)` ≈ paper Subtotal (±0.50€).
- amount_total ≈ paper Total ±1€.
- Все строки имеют supplier_sku, expected_qty, supplier_product_name, item_comment.
...
|---|---|---|
| 🤖 | 5 (light blue) | Робот разрулил clean |
| ✅ | 10 (green) | Paper match clean |
| 🟡 | 3 (yellow) | Minor auto-fix |
| 🟠 | 2 (orange) | Substantial auto-fix |
| ❌ | 4 (red) | Блокер |
...
### 9.1 Skip правила
- `state ≠ 'draft'` → skip pedido (закрыт или cancel).
- `state == 'draft'` + `x_studio_claude_finalize=True` → automation в работе, skip.
- На line `item_comment` содержит «✅ Verified by Claude AI <дата>» → skip line.
...
### 13.1 Текущая палитра
- 1 = default (нет данных)
- 3 = yellow (warning)
- 4 = red (error)
- 10 = green (OK)

### 13.2 Новые уровни (после расширения)
- **5** = blue/light blue (🤖 робот разрулил clean)
- **2** = orange (🟠 substantial auto-fix)

### 13.3 Decision logic (псевдокод)
```python
if line had wrong-card fix or pack-conversion or new-card-created or split-MIX:
    color = 2  # orange
elif line had minor qty fix (≤5) or price update:
    color = 3  # yellow
elif paper.qty == florist.qty and price match and codigo learned:
    color = 10  # green
elif line clean auto by bot (positional + obloecho codigo, no fixes):
    color = 5  # blue
elif unresolved issue:
    color = 4  # red
else:
    color = 1  # default
```
...
### 13.4 Mirror
`master-context/review_status_automation.py` (existing) → расширить с blue/orange branches. Owner финализирует через Studio.
