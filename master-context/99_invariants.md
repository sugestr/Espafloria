...
### 2. Бумага поставщика — НЕ источник истины
Это **одно из трёх** представлений. Вместе с:
- `x_studio_expected_qty` (логист)
- `stock.move.quantity` (флорист)

Все три хранятся независимо.
...
### 25. Rolling inventory для ваз
При продаже товара `DECORACION` / `EMBALAJE` / `PRODUCTOS ESPECIALES/DECORACION` — требовать пересчёт остатка в магазине. Не реализовано, но заложено как правило.

---

## 🏭 Инфраструктура

### 26. Odoo.sh Custom — one-way migration
Нельзя вернуться на Standard. Обдумывай любое решение, которое требует custom fields / Studio / addons.

### 27. Make.com бот = 19 Odoo calls на pedido
1 Worker × ~300ms/call × 19 = ~6s на bot-обработку. При пиках может быть bottleneck — следить за 502.
...
### 35. Бумага ≠ факт ≠ логист. Odoo calc не управляет реальностью
Усиленная версия инварианта 2. Принцип: Odoo — это **инструмент отражения** реальности магазина, а не её диктатор.
Система **не должна** автоматически писать `quantity` или `packs` за флориста на основе расчёта. Только человек решает, сколько реально принято.
...
> **Paper ≠ Truth. Receipt = Truth. Logist = hint. -1 ≠ 0.**
...
- Все файлы 01-09 — контекст применения инвариантов
