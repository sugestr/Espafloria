...
2. В репо (commit через sandbox-delivery)
...
- **Blueprint:** хранится только в Make.com (prod scenario). Достаём через Make MCP (`mcp:make` → `scenarios_get`). В репо не лежит (~230 KB, дубликат бесполезен).
- **OCR prompt:** `prompt_ocr_v1.txt`
- **Reconciliation engine:** `prompt_reconciliation_v3.5.txt` (**prod**)
- **Diagnostics:** `prompt_diagnostics_v3.1.txt` (**prod**)
- **Line-log шаблоны:** `make_line_log_unit.txt`, `make_line_log_pack.txt`
...
Полный текст: `prompt_ocr_v1.txt`
