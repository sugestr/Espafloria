...
### 3.1 Action 1217 v5 (`master-context/reconcile_finalize_action.py`)
...
- `fileId` — из DOM iframe attachment
...
### 6.4 Список 16 mongoId для PNG-плейсхолдеров
```
12492439 → 69d032da49ae8a0ada04c61d  (✓ скачан как verdnatura_12492439 (1).pdf)
...
```
Для остальных 157 закрытых pedidos paper PDF тоже надо подгрузить в `ir.attachment` (см. §7.2).

### 6.5 Прицепление PDF в ir.attachment
```python
import base64
with open(f'/Users/andriy/Downloads/<filename>.pdf', 'rb') as f:
...
```
Это P0 для всех 173 — audit trail, paper в Odoo навсегда.
...
- `SESSION_HANDOVER_2026-04-29.md` — **этот документ** (главный baseline)
- `CHANGELOG.md` — последние 15 записей о работе (читать 3-5 верхних)
- `reconcile_finalize_action.py` v5 — текущий код action 1217
