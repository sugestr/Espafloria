...
> 🟡 **STATUS 2026-05-02 (post-reset):** база Odoo обнулена (все pedidos/sales/stock стёрты, каталог переимпортирован из Holded). Текущая v19 morally **требует verify** на чистом каталоге — некоторые правила опираются на pre-reset state (§A5 «existing robo-cards», migrated v2.2 cards). Owner планирует следующей сессией: (a) сравнить v1 vs v19, (b) тестовый прогон через `INSTR_run_reception_algorithm_single_pedido.md` на 1 pedido, (c) выпуск v20 если потребуется.
...
Mirrored в `master-context/reception_action_1217.py`. Триггер: write `x_studio_claude_finalize=True` на `purchase.order`.
...
- [ ] У меня есть доступ к: (a) этот файл (`reception_algorithm.md`), (b) `reception_action_1217.py` (mirror action 1217 в проде, v7.7 contract), (c) `review_status_automation.py` (mirror action 1146 в проде, color/review_status logic — referenced from §B7)
