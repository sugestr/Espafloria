...
5. опционально supplierinfo learning import, если mapping безопасен;
...
- Supplierinfo learning may still be generated for accepted mappings.

For NO_BOOKKEEPER_MODE final output may include:

1. PO import;
2. reconciliation report;
3. optional supplierinfo learning import;
...
Для supplierinfo import Vendor тоже должен быть:
...
5. 5_supplierinfo_learning_import.xlsx
   Опционально, только если mapping безопасен.
...
6. Supplier Pricelist = learned matching hint.
...
5. Supplierinfo learning match by Vendor + Supplier Identity Code/Key.
...
x_studio_supplier_identity_code / key:

Composite semantic identity для future matching.
...
Vendor + supplier_identity_code/key → Odoo product/SKU.
...
Vendor + Supplier Identity Code/Key → Odoo product/SKU.

Accepted identity field aliases:
...
Generate 5_supplierinfo_learning_import.xlsx only for accepted mappings:

- no red blockers;
- product.product External ID exists;
- Supplier Identity Code/Key is stable enough;
- no same identity → different product conflict;
- Vendor exactly SERVIFLOR VILASSAR SL.;
- Vendor Product Code empty unless real reusable supplier code exists.

For every accepted line, generate or update learning evidence.

Supplier Identity Code/Key should encode stable supplier semantics:
...
If same Supplier Identity Code/Key maps to same Odoo product across several imports:

- reinforce mapping.

If same Supplier Identity Code/Key maps to different Odoo products:
...
If Supplier Pricelist export contains no rows for Vendor = SERVIFLOR VILASSAR SL., treat it as structure/reference only, not as Serviflor learning evidence.

====================================================================
19. ITEM COMMENT POLICY
====================================================================
...
====================================================================
21. INTERNAL TRANSFER IMPORT FORMAT — CONFIRMED WORKING
====================================================================
...
====================================================================
24. DONE INTERNAL TRANSFER CORRECTION POLICY
====================================================================
...
====================================================================
25. CREDIT NOTES / RECTIFICATIVAS
====================================================================
...
32. Same Supplier Identity Code/Key maps to different Odoo products and cannot be resolved.
...
15. Supplier Identity Code/Key incomplete.
16. Photo missing.
...
25. New supplier identity learned from accepted mapping.
26. Supplier Pricelist contains no Serviflor rows and is used as structure/reference only.
...
- Supplier Identity Code/Key = future semantic key.
...
====================================================================
28. QUESTIONS TO ASK ONLY IF UNRESOLVED
====================================================================
...
- Whether Supplier Identity belongs to supplierinfo/learning.
...
32. whether Supplier Identity Code/Key exists;
33. detected pack lines count;
34. red blockers count;
35. orange review count;
36. Gloria transfer line count where Gloria actual units > 0, or transfer skipped reason;
37. Blau transfer line count where Blau actual units > 0, or transfer skipped reason;
38. duplicate-SKU risk count;
39. supplierinfo learning candidate count;
40. PO commercial gate status:
    factura base vs generated PO untaxed;
41. confirmation that transfer row-level validation passed, if transfers generated:
    Operations/Quantity == store actual units.

====================================================================
30. RECONCILIATION REPORT CONTRACT
====================================================================
...
- supplierinfo_conflicts must show same Supplier Identity Code/Key → different Odoo products conflicts.
- post_mortem must explain root cause if a previous run failed.

====================================================================
31. TRANSFER FILE AUDIT SHEETS
====================================================================
...
====================================================================
32. SUPPLIERINFO LEARNING IMPORT FILE
====================================================================

5_supplierinfo_learning_import.xlsx is optional.

Generate only if safe.

Possible columns:

- Vendor or partner_id
- Product / External ID or product_id/id
- Vendor Product Name / product_name
...
- Supplier Identity Code / x_studio_supplier_identity_code
- Supplier Identity Key / x_studio_supplier_identity_key

Use only fields that actually exist in supplied Supplier Pricelist export / known Odoo layout.
Do not invent new Studio fields.

Rules:

- Vendor exactly SERVIFLOR VILASSAR SL.
- one row per unique Supplier Identity Code/Key + product mapping;
- duplicates can be collapsed if same identity maps to same product;
- do not collapse if same identity maps to different product — flag 🔴 or 🟠;
- Vendor Product Code empty unless real reusable supplier code exists;
- Supplier Identity Code/Key is not Entrega;
- for pack lines, supplierinfo Unit should reflect supplier purchase UoM if supported;
- do not import supplierinfo if field availability is uncertain;
- do not include uncertain mappings just to “teach” the system.
...
17. Import 5_supplierinfo_learning_import.xlsx only if generated and mapping is accepted.
