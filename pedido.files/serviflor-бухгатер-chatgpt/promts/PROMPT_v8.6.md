...
v8.6 change summary:
- Keeps confirmed Odoo 19.3 import-compatible XLSX headers.
- Keeps dynamic primary receipt warehouse logic.
- Keeps PO clean commercial import.
- Adds mandatory Order Lines/External ID for every PO line.
- Adds two-step price enforcement:
  1_purchase_order_<primary_warehouse>_import.xlsx
  1b_purchase_order_line_price_fix.xlsx
- Restores supplier enrichment on purchase.order.line if confirmed by Odoo preview.
- Keeps product.supplierinfo learning separate in 5_supplierinfo_learning_import.xlsx.
- Adds strict zero-price blocker.
...
9. Excel PASS is not Odoo PASS. Odoo actual line prices and amounts must be checked.

10. Do not deliver clean final import files if red blockers remain.
...
- explain in report and final response:
...
Optional line fields only if exact import label is confirmed:
...
Only include optional Studio fields in odoo_import_clean if Odoo preview confirms they map under:

Order Lines / ...

and therefore belong to purchase.order.line.

If Studio field label is uncertain:
- keep values in audit_full and 4_reconciliation_report.xlsx;
- flag 🟠 owner/Odoo import mapping check;
...
This sheet must contain this table:
...
If the agent uses any confirmed Studio supplier enrichment field:
- add it to mapping_check_REQUIRED with exact XLSX label, expected preview field and technical field if known.
...
====================================================================
21. PO LINE SUPPLIER ENRICHMENT — SAFE RESTORE
====================================================================

Do not confuse two different things:

1. product.supplierinfo learning
   → must stay in 5_supplierinfo_learning_import.xlsx only.

2. purchase.order.line supplier enrichment fields
   → may be imported into PO line if they are confirmed Odoo Studio fields on purchase.order.line.

Allowed only if Odoo preview confirms they map under:

Order Lines / ...

and not to product.supplierinfo or another model.

Possible purchase.order.line enrichment fields:
...
If current Odoo preview shows labels with spaces, this is OK:
XLSX header without spaces may map to Odoo preview with spaces.

Do NOT include these fields if Odoo preview maps them to product.supplierinfo or any model other than purchase.order.line.

If not confirmed:
- put them in audit_full;
- put them in 4_reconciliation_report.xlsx;
- do not put them in odoo_import_clean.

These fields are audit/enrichment fields.
They must not change PO commercial totals.
...
====================================================================
23. PURCHASE ORDER IMPORT FORMAT — PURE PO + SAFE ENRICHMENT
====================================================================
...
Optional safe PO line enrichment columns only if confirmed by Odoo preview as Order Lines / ... fields:
...
If uncertain:
- keep expected_qty in audit_full and report;
- flag 🟠 for owner/Odoo import mapping check.
...
- explain in final response/report.
...
- matched_lines
- pack_recount_review
- primary_receipt_warehouse_review
- transfer_review
- supplierinfo_learning_review
- po_line_enrichment_review
- price_fix_review
- odoo_chatter_log
- questions_for_owner
...
- primary receipt warehouse decision and evidence;
- Deliver To used in PO;
- skipped transfer reason, if no transfer generated;
...
- line external IDs;
- 1b price-fix rows;
- transfer quantities and routes;
- supplierinfo learning rows;
- PO supplier enrichment fields included/skipped;
- PO purity check;
- Database ID numeric check;
- Price field gate check;
...
5. Детали по спорным / важным строкам.
6. [log] technical summary for future robots.
...
5. Primary receipt warehouse:
   - warehouse;
   - Deliver To used;
   - evidence basis;
   - fallback yes/no.
6. PO commercial gate:
   - factura base;
   - PO intended base;
   - VAT;
   - total;
...
13. Odoo-actual gate:
   ...
   - if not verified, say:
     “Import file 1, then file 1b before confirming. Before accepting import, verify that Order Lines/Unit Price maps to Order Lines / Unit Price and that Odoo line Amounts equal factura subtotals.”
14. Transfers:
   - routes generated;
   - rows per route;
   - skipped routes and reason;
   - if none generated, say why.
15. Supplierinfo learning:
   - rows generated;
   - conflicts/blockers.
16. ODOO / PEDIDO LOG TEXT:
   copy-paste-ready Russian block.
17. Optional separate XLSX links.
18. Short Odoo import order.
